...
def generate_gap_junctions(connection_prob, coupling_coeffs, coupling_params, ranstream_gj, gids_a, gids_b, gj_probs, gj_distances, cell_dict_a, cell_dict_b, gj_dict):
...
def generate_gj_connections(env, forest_path, soma_coords_dict,
    ...
    for (i, (pp, gj_config)) in enumerate(gj_config_dict.iteritems()):
        ...
        clst_a = []
        gid_a  = []
        for (gid, coords) in soma_coords_dict[population_a].iteritems():
        ...
        clst_b = []
        gid_b  = []
        for (gid, coords) in soma_coords_dict[population_b].iteritems():
        ...
        gids_a = []
        gids_b = []
        for gid, v in gj_prob_dict.iteritems():
            if gid % size == rank:

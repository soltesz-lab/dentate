...
ibrun  python3 network_clamp.py optimize  -c Network_Clamp_MC_SLN.yaml \
    ...
    --input-features-path $RAIKOVSCRATCH/striped/dentate/Full_Scale_Control/DG_input_features_20200611_compressed.h5 \
    --input-features-namespaces 'Place Selectivity' \
    --input-features-namespaces 'Grid Selectivity' \
    --input-features-namespaces 'Constant Selectivity' \
    --config-prefix config  --opt-iter 100 --param-config-name 'Weight no GC MC' \
    --arena-id A --trajectory-id Diag \
    --target-features-path $RAIKOVSCRATCH/striped/dentate/Slice/MC_extent_input_spike_trains_20201001.h5 \
    selectivity_rate

#    --target-features-path $RAIKOVSCRATCH/striped/dentate/Slice/DG_MC_input_spike_trains_20200708.h5 \

...
"""
import os, sys, copy, pprint, time, gc
        ...
        if trial_regime == "mean":
            return {
                gid: -mean_trial_rate_mse(
                    gid, firing_rate_vectors_dict[gid], target_rate_vector_dict[gid]
                )
                for gid in my_cell_index_set
            }
        elif trial_regime == "best":
    ...
    if results_file is None:
        if env.results_path is not None:
            file_path = os.path.join(
                env.results_path, f"distgfs.network_clamp.{env.results_file_id}.h5"
            )
        else:
    ...
    if opt_results is not None:
        if ProblemRegime[problem_regime] == ProblemRegime.every:
            ...
            for gid, opt_result in viewitems(opt_results):
                ...
                logger.info(
                    f"Optimized parameters and objective function for id {gid}: "
                    f"{pprint.pformat(gid_result_params_dict[int(gid)])} @"
                    f"{result_value}"
                )
            results = {pop_name: gid_result_params_dict}
        else:
    ...
    if len(param_path) > 0:
        for this_param_path, pop_param_tuple_dict in zip(
            ...
            task_id = controller.submit_call(
                "dist_run",
                module_name="dentate.network_clamp",
                args=(
                    init_params,
                    cell_index_set,
...
def write_output(env):
    ...
    io_utils.recsout(
        env, env.results_file_path, write_cell_location_data=True, write_trial_data=True
    )
    if rank == 0:
    ...
    if size > 1:
        ...
        if distwq.is_controller:
            distwq.run(
                ...
                module_name="dentate.network_clamp",
                verbose=True,
                args=(init_params, cell_index_set, params_path, pop_params_tuple_dicts),
                spawn_workers=True,
                nprocs_per_worker=1,
            )
...
def optimize(
    ...
    results_dict, distgfs_params = optimize_run(
        ...
        return_distgfs_metrics=True,
        verbose=verbose,
    )
    if results_dict is not None:
        if results_path is not None:
            file_path = os.path.join(
                results_path, "network_clamp.optimize.{results_file_id}.yaml"
            )
            write_to_yaml(file_path, results_dict.parameters)
    comm.barrier()
    return results_dict, distgfs_params
...
@click.option(
    "--arena-id", "-a", required=False, type=str, help="arena id for input stimulus"
)
@click.option(
    "--trajectory-id",
    "-t",
...
@click.option(
    "--opt-seed", type=int, help="seed for random sampling of optimization parameters"
)
@click.option(
...
cli.add_command(go_cmd, "go")
cli.add_command(optimize_cmd, "optimize")

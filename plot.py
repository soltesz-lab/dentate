...
## Plot activity ratio between a stimulus population and a response population
## Based on sparseness measure developed by Rolls and Tovee 1995.
def plot_activity_ratio, namespace_id, stimulus_population, response_population, timeRange = None, maxSpikes=int(1e6), binSize = 5., 
                          overlay=True, figSize = (15,8), fontSize = 14, lw = 3, 
                          saveFig = None, showFig = True, verbose = False): 
    ''' 
    Plot activity ratio between a stimulus population and a response population.
        ...
        - input_file: file with spike data
        - namespace_id: attribute namespace for spike events
        - stimulus_population: name of population representing stimulus
        - response_population: name of population representing response
        - timeRange ([start:stop]): Time range of spikes shown; if None shows all (default: None)
        - binSize (int): Size in ms of each bin (default: 5)
        - overlay (True|False): Whether to overlay the data lines or plot in separate subplots (default: True)
        - figSize ((width, height)): Size of figure (default: (10,8))
    ...
    spkdata = spikedata.read_spike_events (comm, input_file, [stimulus_population, response_population],
                                           namespace_id, timeRange=timeRange, maxSpikes=maxSpikes, verbose=verbose)

...
#### TO DO: PULL THE dtype directly
        ...
        for fidx, filobj in enumerate(self.fil_arr[:,0]):
            for cidx, cell in enumerate(self.fil_arr[fidx, 1]):
                ...
                self.obj_val_mean_arr[fidx, cidx, :, 0] = np.array(ref_features['mean_infld_rate']) 
                self.obj_val_mean_arr[fidx, cidx, :, 1] = np.array(ref_features['mean_outfld_rate']) 
                self.obj_val_mean_arr[fidx, cidx, :, 2] = np.array(ref_features['mean_outfld_state']) 
...
#                self.bestmean_prm[fidx, cidx] = ref_cell['parameters'][self.bestmean_idx[fidx, cidx]]
    ...
    def get_fig_axes(self):
        ...
        obj_axes = np.empty(shape=objspec.get_geometry(), dtype='O')
        ...
        for idx, ax in np.ndenumerate(obj_axes): 
            obj_axes[idx] = fig.add_subplot(objspec[idx])
...
#                        opr_axes[0,i].scatter(self.best_idx[j,cidx,0,tidx], self.best_prm[prm][j,cidx, 0, tidx],  marker=plotshap[j], color=colors[cidx], s=4)
        ...
        for fidx, filobj in enumerate(self.fil_arr[:,0]):
            for cidx, cell in enumerate(self.fil_arr[fidx, 1]):
             ...
             #   print(opt_val_arr[fidx,cidx])
                for objidx in range(self.N_objectives):
                    ref_ax = opt_axes[objidx,cidx]
                    ref_ax.scatter(N_iter_arr, self.obj_val_mean_arr[fidx, cidx,:,objidx], marker=plotshap[fidx], color='k', facecolors='none', lw=1, s=3, zorder=200)
#                    ref_ax.axhline(self.target[0], color='k', ls='--', zorder=0.25)
#                    for tidx in range(self.N_trials):
#                        ref_ax.scatter(N_iter_arr, opt_val_arr[fidx,cidx,:,objidx,tidx], marker=plotshap[fidx], color=colors[tidx], facecolors='none', lw=1, s=3, zorder=1)
...
#            ref_ax.set_ylim(top=ytop, bottom=0)
        ...
        opt_axes[0,0].set_ylabel('Mean Infld \n Rate [spikes/s]')
        opt_axes[1,0].set_ylabel('Mean Outfld \n Rate [spikes/s]')
        opt_axes[2,0].set_ylabel('Mean Outfld \n State [mV]')
        opt_axes[0,0].set_xlabel('Iteration Index')

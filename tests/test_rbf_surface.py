...
du = (1.01*np.pi-(-0.016*np.pi))/max_u*obs_u_spatial_resolution
dv = (1.425*np.pi-(-0.23*np.pi))/max_v*obs_v_spatial_resolution
obs_u = np.arange(-0.016*np.pi, 1.01*np.pi, du)
obs_v = np.arange(-0.23*np.pi, 1.425*np.pi, dv)
print 'obs_v size: ', obs_v.size
...
print 'uvl_obs shape: ', uvl_obs.shape

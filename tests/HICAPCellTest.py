...
def ap_test (template_class, tree, v_init):
    ...
    f=open("HICAPCell_ap_results.dat",'w')
...
def fi_test (template_class, tree, v_init):
    ...
    f=open("HICAPCell_fi_results.dat",'w')
...
def synapse_group_test (env, presyn_name, gid, cell, syn_obj_dict, syn_params_dict, group_size, v_holding, v_init, tstart = 200.):
    ...
    for section_name, sec_syn_params_dict in utils.viewitems(syn_params_dict):
        for syn_name in sec_syn_params_dict:
            ...
            if syn_name == 'SatAMPA':
                v = hoc_cell.syntest_exc(tstart,v_holding,v_init,"HICAPCell_%s_%s_synapse_trace_%i.dat" % (presyn_name, syn_name, group_size))
            elif syn_name == 'AMPA':
                v = hoc_cell.syntest_exc(tstart,v_holding,v_init,"HICAPCell_%s_%s_synapse_trace_%i.dat" % (presyn_name, syn_name, group_size))
            elif syn_name == 'SatGABA':
                v = hoc_cell.syntest_inh(tstart,v_holding,v_init,"HICAPCell_%s_%s_synapse_trace_%i.dat" % (presyn_name, syn_name, group_size))
            elif syn_name == 'GABA':
                v = hoc_cell.syntest_inh(tstart,v_holding,v_init,"HICAPCell_%s_%s_synapse_trace_%i.dat" % (presyn_name, syn_name, group_size))
            ...
            f=open("HICAPCell_%s_%s_synapse_results_%i.dat" % (presyn_name, syn_name, group_size), 'w')
...
def synapse_group_rate_test (env, presyn_name, gid, cell, syn_obj_dict, syn_params_dict, group_size, rate, tstart = 200.):
    ...
    for section_name, sec_syn_params_dict in utils.viewitems(syn_params_dict):
        for syn_name in sec_syn_params_dict:
            ...
            f=open("HICAPCell_%s_%s_synapse_rate_%i.dat" % (presyn_name, syn_name, group_size),'w')

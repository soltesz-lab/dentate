...
def passive_test (tree, v_init):
    ...
    cell = utils.new_cell ("NGFCell", neurotree_dict=tree)
    h.dt = 0.025
    ...
    f=open("NGFCell_passive_results.dat",'w')
...
def gap_junction_test (tree, v_init):
    ...
    cell1 = utils.new_cell ("NGFCell", neurotree_dict=tree)
    cell2 = utils.new_cell ("NGFCell", neurotree_dict=tree)
...
def main(template_path,forest_path):
    ...
    h.xopen(template_path+'/NGFCell.hoc')
    h.pc = h.ParallelContext()
...
if __name__ == '__main__':
    main(args=sys.argv[(utils.list_find(lambda s: s.find("NGFCellTest.py") != -1,sys.argv)+1):])

...
def organize_syn_ids_by_source(gid, env, syn_ids=None):
    ...
    :param syn_ids: array of int
    """
    source_names = {id: name for name, id in list(env.pop_dict.items())}
...
def insert_syns_from_mech_attrs(gid, env, postsyn_name, presyn_name, syn_ids, unique=False):
    ...
    for syn_id in syn_ids:
        ...
        for syn_name, mech_params in list(syn_params.items()):
...
def mksyns(gid, cell, syn_ids, syn_params, env, edge_count, add_synapse=add_shared_synapse):
    ...
    for syn_id, syn_index in zip(syn_ids, syn_indexes):
        ...
        for syn_name, params in list(syn_params.items()):
...
def make_synapse_graph(syn_dict, neurotree_dict, return_root=False):
    ...
    for sec_id, syn_id_locs in list(sec_syn_dict.items()):
...
def synapse_seg_density(syn_type_dict, layer_dict, layer_density_dicts, seg_dict, seed, neurotree_dict=None):
    ...
    if neurotree_dict is not None:
        ...
        secnodes_dict = None
    for (syn_type_label, layer_density_dict) in list(layer_density_dicts.items()):
        syn_type = syn_type_dict[syn_type_label]
        rans = {}
        for (layer_label, density_dict) in list(layer_density_dict.items()):
        ...
        for sec_index, seg_list in list(seg_dict.items()):
            for seg in seg_list:
                L = seg.sec.L
                nseg = seg.sec.nseg
                if neurotree_dict is not None:
...
def synapse_seg_counts(syn_type_dict, layer_dict, layer_density_dicts, sec_index_dict, seg_dict, seed, neurotree_dict=None):
    ...
    for (syn_type_label, layer_density_dict) in list(layer_density_dicts.items()):
        ...
        segcounts = []
        layers = []
        for sec_index, seg_list in list(sec_dict.items()):
            for seg in seg_list:
                L = seg.sec.L
                nseg = seg.sec.nseg
                if neurotree_dict is not None:
...
def distribute_uniform_synapses(density_seed, syn_type_dict, swc_type_dict, layer_dict, sec_layer_density_dict, neurotree_dict,
    ...
    segcounts_per_sec = {}
    for (sec_name, layer_density_dict) in list(sec_layer_density_dict.items()):
        ...
        seg_dict = {}
        for (sec_index, sec) in list(sec_dict.items()):
            seg_list = []
            if maxdist is None:
                for seg in sec:
        ...
        sample_size = total
        for (syn_type_label, _) in list(layer_density_dict.items()):
            syn_type = syn_type_dict[syn_type_label]

...
RANGE gmax, ghyhtfbar, ghyhtsbar
...
PARAMETER {
	...
	?ghysbar (mho/cm2) // taken this out as they are the same...
	ehyf (mV)
	ehys (mV)
	ghyhtfbar (mho/cm2)
	ghyhtsbar (mho/cm2)
...
BREAKPOINT {
	...
	ghyf = gmax * hyf*hyf  ? Know reusing gmax is bad, but short term measure
	ihyf = ghyf * (v-ehyf)
    
	ghys = gmax * hys*hys  ? Know reusing gmax is bad, but short term measure
	ihys = ghys * (v-ehys)
